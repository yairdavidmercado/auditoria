...
export class FormComponent implements OnInit {
  ...
  finalizar(){
    ...
    if (this.explicacion.trim().length < 150) {
      sweetAlert("Su explicación solo tiene "+this.explicacion.trim().length+" caracteres, recuerde que debe superar los 150 caracteres para poder continuar.")
      return false;
    }
  ...
  hallazgo(id_auditoria:string = '0'){
    ...
    this._storeServises.loading = true
    this._FormService.hallazgo(params).subscribe(
      resp => [
        this.dataHallazgo = resp,
        console.log(resp)
      ], 
      err => [
        //this._storeServises.loading = false,
